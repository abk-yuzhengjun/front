<template xmlns:height="http://www.w3.org/1999/xhtml">
  <div style="width: 100%;height:100%">
    <div style="width: 100%;display: flex;padding-bottom: 10px; background: #F3F4F7">
      <div style="width: 100%; background: #FFFFFF;display: flex; padding: 2px 20px 2px 20px;flex-direction: column;">
        ...
        <div
          ...
          <span>案件编号:&nbsp; {{ case_name }}</span>
          <span>案件名 : &nbsp;{{ case_name }}</span>
          <span>状态 :&nbsp; 未开始</span>
          <span>
            <!--            <el-button type="text" @click="remId(scope.$index)">创建案件</el-button>-->
            <a style="text-decoration: underline" @click="editTaskInfo">编辑案件</a>
          </span>
        </div>
    ...
    <div
      ...
      <el-input v-model="tableDataName" placeholder="请输入" suffix-icon="el-icon-search" style="width:360px"
                @input="doFilter"/>
    </div>
    ...
    <div style="width: 100%;height:65%; display: flex;flex-direction: column;padding:10px;">
      ...
      >
        ...
        <el-table-column label="任务名" min-width="10px" align="center">
          <template slot-scope="scope">
            <el-button type="text" @click="jumpToEvidenceInformation(scope.$index,scope.row)">{{scope.row.task_name}}</el-button>
          </template>
        </el-table-column>
        <el-table-column property="task_detail" label="任务信息" min-width="20px" align="center"/>
        ...
        <el-table-column label="状态" min-width="10px" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="" align="center" min-width="8px">
          <template slot-scope="scope">
            <el-button size="mini" type="primary" v-if="scope.row.task_status === 'ready'">开始</el-button>
            <el-button size="mini" type="warning" v-else>继续</el-button>
...
<script>
    ...
    import TaskCreator from '../casemanager/creattask'
    ...
    export default {
        ...
        computed: {
            taskData() {
                console.log('evidence computed run')
        ...
        methods: {
            init() {
                ...
                console.log('init evidencedisplay1')
                if (this.bondsAllList !== undefined && this.bondsAllList !== '') {
                    console.log('init evidencedisplay2')
                    this.getCreateTable()
                }
            },
            getMessage() {
            ...
            editTaskInfo(index, row) {
                ...
                this.$store.commit('caseInfo/setDialogRow', row)
            },
            getCreateTable() {
                ...
                this.$router.push(
                    {
                        name: 'evidencedisplay',
                        query: {
                            treeTempList: this.tempTreeData
                        }
                    })
            },
            jumpToCaseDisplay() {
